    ...
    public FXMLLoader startShift(String username, String password) throws Exception {
        try (UserPrivilege userPrivilege = userAuth.login(username, password)) {
            ...
            if(userAuth.hasRole(Role.MAINTENANCE.name())){
                ...
                fxmlLoader = ViewFactory.getHome();
                fxmlLoader.setControllerFactory(controller -> new Controller(agent));
        ...
        for (TicketsDto ticketsDto : list) {
            ...
            switch (ticketType) {
                ...
                case PAID:
                    ...
                    noOfAdjusted++;
                    if(Objects.equals(ticketsDto.getPaymentMode(), "CASH")){
                        noOfPaidExitCash++;
                        paidExitCashAmount+=(int)ticketsDto.getAmount();
                    }else if(Objects.equals(ticketsDto.getPaymentMode(), "UPI")){
                        noOfPaidExitUpi++;
                        paidExitUpiAmount+=(int)ticketsDto.getAmount();
                    }else if(Objects.equals(ticketsDto.getPaymentMode(), "POS")){
                        noOfPaidExitPos++;

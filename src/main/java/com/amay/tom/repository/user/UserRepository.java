...
//            "updatedAt DATETIME" +
    ...
    protected static final String CREATE_USER_PRIVILEGE_TABLE_SQL =
            "CREATE TABLE IF NOT EXISTS " + USER_PRIVILEGE_TABLE_NAME + " (" +
                    "username VARCHAR(255) PRIMARY KEY," +
                    "bnrCashAdd BOOLEAN," +
                    "bnrTesting BOOLEAN," +
                    "bnrMoveCash BOOLEAN," +
                    "checkAvailableCash BOOLEAN," +
                    "coinRefill BOOLEAN," +
                    "coinModuleTesting BOOLEAN," +
                    "coinDumping BOOLEAN," +
    ...
    protected static final String INSERT_USER_PRIVILEGE_SQL =
            "INSERT INTO " + USER_PRIVILEGE_TABLE_NAME + " (" +
                    "username, bnrCashAdd, bnrTesting, bnrMoveCash, checkAvailableCash, coinRefill, coinModuleTesting, coinDumping, peripheralTest, configuration, modeSettings, versionCheck, importAndExport, shutdownAndRestart" +
                    ") VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

    protected static final String SELECT_USER_PRIVILEGE_BY_USERNAME_SQL =
            "SELECT * FROM " + USER_PRIVILEGE_TABLE_NAME + " WHERE username = ?";

    protected static final String UPDATE_USER_PRIVILEGE_SQL =
            "UPDATE " + USER_PRIVILEGE_TABLE_NAME + " SET " +
                    "bnrCashAdd=?, bnrTesting=?, bnrMoveCash=?, checkAvailableCash=?, coinRefill=?, coinModuleTesting=?, coinDumping=?, peripheralTest=?, configuration=?, modeSettings=?, versionCheck=?, importAndExport=?, shutdownAndRestart=? " +
                    "WHERE username=?";

    protected static final String DELETE_USER_PRIVILEGE_BY_USERNAME_SQL =
            "DELETE FROM " + USER_PRIVILEGE_TABLE_NAME + " WHERE username = ?";

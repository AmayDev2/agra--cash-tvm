...
public class OfflineQRTicketGenerator extends AbstractQRTicketGenerator {
    ...
    public QRTicketGenerator create(Agent agent) {
        return new com.amay.tom.service.qrService2.impl.OfflineQRTicketGenerator(agent.getTicketsRepository(),agent.getTransactionRepository(),agent.getScuService(),agent.getShift(),agent.getThreadPool(),agent.getCcuService(),agent.getShiftIdGeneratorService().getTicketIdGeneratorService(), agent.getMasterConfigInfo());
    }

...
public  abstract class QRTicketGenerator implements QRTicketService {
    ...
    private void pushToCcu(String orderId, String transactionId, PostGeneratedTicket postGeneratedTicket) {
    ...
    protected void pushToScuAsync(String orderId, String transactionId, PostGeneratedTicket postGeneratedTicket) {
        Logger.debug("Pushing ticket issue info to SCU asynchronously");
        CompletableFuture.runAsync(() -> pushToScu(orderId, transactionId, postGeneratedTicket),threadPool.getSingleThread());
    }

    protected void pushToCcuAsync(String orderId, String transactionId, PostGeneratedTicket postGeneratedTicket) {
        Logger.debug("Pushing ticket issue info to CCU asynchronously");
        CompletableFuture.runAsync(() -> pushToCcu(orderId, transactionId, postGeneratedTicket),threadPool.getSingleThread());
    }
    ...
    protected void verifyPayment(String orderId, String transactionId, int amount) {
        ...
        //if(amount==0) throw new PaymentNotDoneException("Payment is not done for the order id: "+orderId);
    }

    protected ArrayList<GeneratedTicket> pushTicket(String finalOrderId, String finalTransactionId, List<PostGeneratedTicket> postGeneratedTickets){
        ...
        postGeneratedTickets.forEach(postGeneratedTicket -> {
            ...
            pushToScuAsync(finalOrderId, finalTransactionId,postGeneratedTicket);
            pushToCcuAsync(finalOrderId, finalTransactionId,postGeneratedTicket);
        });
